...
class HairMod(loader.Module):
    ...
    async def haircmd(self, message):
        ...
        if not reply:
            return await message.edit("No reply on the picture/sticker.")
        if reply.file.mime_type.split('/')[0] == "image":
            im = Image.open(io.BytesIO(await message.client.download_file(reply, bytes)))
            draw = ImageDraw.Draw(im)
            w, h = im.size
            draw.arc((r(w), r(h), r(w) + r(300), r(h) + r(550)),
                     r(350), 180 + r(350), fill="black", width=1)
            out = io.BytesIO()
            ...
            return await message.edit("This is not a picture/sticker.")
